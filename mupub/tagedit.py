...
def get_copyright(cc_name, date):
    ...
    return template.substitute(
        TIMESTAMP=lic.get_timestamp(date.year), LICENSE_URL=lic.get_url()
    )
...
def _validate_id(mu_id, query=True):
    ...
    if query:
        try:
            mu_id = int(
                prompt.query(
                    "Numeric ID for this piece (ctrl-d to abort)",
                    default=str(mu_id),
...
def _augmented_table(table, muid, query=True):
    ...
    if "footer" in table:
        try:
            ...
            logger.warning(
                'Ignoring footer ("%s") assuming new entry.' % table["footer"]
            )
            pubdate = date.today()
            mu_id = 0
...
def _mark_tag_as_used(mu_id):
    ...
    with sqlite3.connect(mupub.getDBPath()) as conn:
        conn.execute(
            "INSERT OR REPLACE INTO id_tracker (piece_id) VALUES (?)", (piece_id,)
        )
...
def tag_file(header_file, new_id=0, query=True):
    ...
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".ly", prefix="mu_", delete=False
    ) as outfile:
        outfnm = outfile.name
